...
def img_process_to_remove_bk(in_arr_image, in_i_index, in_str_folder_prefix, in_b_train_or_test):    
    ...
    tmp_ideal_arr_image = np.copy(in_arr_image)
    
    #print("\n tmp_arr_image.dtype = ", tmp_arr_image.dtype, tmp_arr_image.shape)   
    
    tmp_ideal_arr_image[tmp_ideal_arr_image <= 254] = 0
    
    
    tmp_ideal_arr_image = apply_rmv_sm_obj(tmp_ideal_arr_image, 0)

    i_ideal_region_count, i_ideal_least_area, i_ideal_least_max_of_wh = img_info_analysis(tmp_ideal_arr_image)
    
    out_fn = in_str_folder_prefix + "_bkr_ideal" + "/" + str(in_i_index) + ".png"
    
    
    #tmp_arr_image = np.reshape(tmp_arr_image, 64 * 64)
    misc_imsave(out_fn, tmp_ideal_arr_image)
    
    
    
    out_fn = in_str_folder_prefix + "_bkr_debug" + "/" + str(in_i_index) + "_ideal" + ".png"
    
    #misc_imsave(out_fn, tmp_ideal_arr_image)
    plt.imsave(out_fn, tmp_ideal_arr_image)
    ...
    if b_do_practical:
        ...
        #pass
        out_arr_image = np.copy(tmp_ideal_arr_image)
